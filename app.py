...
# Conexión persistente con permisos de escritura
try:
...
# --- 2. MOTOR PDF PROFESIONAL (UTF-8 Y SIN CORTES) ---
class PDF_Pro(FPDF):
    def footer(self):
...
def generar_pdf(titulo, perfil, cliente, proy, datos, fotos, img_portada, logo_p):
    ...
    if img_portada:
        ...
        pdf.image("temp_p.jpg", x=45, y=85, w=120, h=100)
...
if not st.session_state['conectado']:
    ...
    if op == "Perfil Empresa":
        ...
        p_data = {"empresa": "TECNOELEC SpA", "rut": ""}
        if os.path.exists(PERFIL_FILE):
            with open(PERFIL_FILE, "r") as f: p_data = json.load(f)
