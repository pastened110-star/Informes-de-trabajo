...
if not st.session_state['conectado']:
    ...
    st.markdown("""
        ...
        .stButton>button { background-color: #ffcc00 !important; color: #001f3f !important; font-weight: bold !important; border-radius: 8px !important; border: none !important; }
        </style>
        """, unsafe_allow_html=True)
else:
    # DISEÑO PANEL INTERNO (AZUL PROFESIONAL Y BOTONES VISIBLES)
    st.markdown("""
        <style>
        .stApp { background-color: #1a2a40 !important; background-image: none !important; }
        
        /* CORRECCIÓN MENÚ LATERAL */
        [data-testid="stSidebar"] { background-color: #0e1a2b !important; border-right: 1px solid #4b4d5a; }
        [data-testid="stSidebar"] * { color: #ffffff !important; }

        /* CORRECCIÓN BOTONES (PARA QUE NO SE VEAN BLANCOS) */
        ...
        /* RECUADRO FOTOS */
        [data-testid="stFileUploadDropzone"] { background-color: #f0f2f6 !important; border: 2px dashed #ffcc00 !important; }
        [data-testid="stFileUploadDropzone"] * { color: #001f3f !important; font-weight: bold !important; }

        h1, h2, h3, p, span, label { color: white !important; }
        .stTextInput>div>div>input, .stTextArea>div>div>textarea, .stSelectbox>div>div>div {
            background-color: white !important; color: #001f3f !important; border-radius: 8px !important;
        }
        </style>
        """, unsafe_allow_html=True)
...
# --- 3. MOTOR PDF COMPLETO (REINTEGRADO) ---
class PDF_Pro(FPDF):
    def footer(self):
...
if not st.session_state['conectado']:
    ...
    if op == "Perfil Empresa":
        ...
        with st.form("fc", clear_on_submit=True):
            ...
            if st.form_submit_button("Guardar Cliente en la Nube"): # BOTÓN CORREGIDO
                if n and r:
        ...
        df_g = obtener_clientes()
        if df_g.empty: st.warning("Registre un cliente primero."); st.stop()
        c_sel = st.selectbox("Seleccionar Cliente", df_g['Nombre'].tolist())
